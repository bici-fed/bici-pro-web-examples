export default {
  // 返回值可以是数组形式
  'GET /api/menu': {
    code: '200',
    msg: 'success',
    data: [
      {
        menuId: 136,
        parentId: 0,
        title: 'Dashboard',
        path: '/dashboard',
        component: null,
        menuType: 0,
        sortNumber: 0,
        authority: null,
        target: '_self',
        icon: 'home-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:00:34',
        updateTime: '2021-11-05 17:33:42',
        children: null,
        checked: null
      },
      {
        menuId: 103,
        parentId: 102,
        title: '查询用户',
        path: null,
        component: null,
        menuType: 1,
        sortNumber: 1,
        authority: 'sys:user:list',
        target: '_self',
        icon: null,
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2020-02-26 12:52:06',
        updateTime: '2020-09-16 11:05:35',
        children: null,
        checked: null
      },
      {
        menuId: 104,
        parentId: 102,
        title: '添加用户',
        path: null,
        component: null,
        menuType: 1,
        sortNumber: 1,
        authority: 'sys:user:add',
        target: '_self',
        icon: null,
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2020-02-26 12:52:06',
        updateTime: '2020-09-16 11:05:35',
        children: null,
        checked: null
      },
      {
        menuId: 105,
        parentId: 102,
        title: '修改用户',
        path: null,
        component: null,
        menuType: 1,
        sortNumber: 1,
        authority: 'sys:user:update',
        target: '_self',
        icon: null,
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2020-02-26 12:52:06',
        updateTime: '2020-09-16 11:05:35',
        children: null,
        checked: null
      },
      {
        menuId: 106,
        parentId: 102,
        title: '删除用户',
        path: null,
        component: null,
        menuType: 1,
        sortNumber: 1,
        authority: 'sys:user:delete',
        target: '_self',
        icon: null,
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2020-02-26 12:52:06',
        updateTime: '2020-09-16 11:05:35',
        children: null,
        checked: null
      },
      {
        menuId: 159,
        parentId: 158,
        title: '个人资料',
        path: '/user/profile',
        component: 'user/profile',
        menuType: 0,
        sortNumber: 1,
        authority: null,
        target: '_self',
        icon: 'user-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:09:58',
        updateTime: '2021-11-05 17:37:17',
        children: null,
        checked: null
      },
      {
        menuId: 113,
        parentId: 112,
        title: '查询菜单',
        path: null,
        component: null,
        menuType: 1,
        sortNumber: 1,
        authority: 'sys:menu:list',
        target: '_self',
        icon: null,
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2020-03-21 16:43:30',
        updateTime: '2020-03-21 18:45:40',
        children: null,
        checked: null
      },
      {
        menuId: 155,
        parentId: 154,
        title: '403',
        path: '/exception/403',
        component: 'exception/403',
        menuType: 0,
        sortNumber: 1,
        authority: null,
        target: '_self',
        icon: 'exclamation-circle-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:08:47',
        updateTime: '2021-11-05 17:36:43',
        children: null,
        checked: null
      },
      {
        menuId: 145,
        parentId: 144,
        title: '基础列表',
        path: '/list/basic',
        component: 'list/basic',
        menuType: 0,
        sortNumber: 1,
        authority: null,
        target: '_self',
        icon: 'table-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:05:10',
        updateTime: '2022-03-11 15:40:17',
        children: null,
        checked: null
      },
      {
        menuId: 173,
        parentId: 172,
        title: '表格实例',
        path: '/example/table',
        component: 'example/table',
        menuType: 0,
        sortNumber: 1,
        authority: null,
        target: '_self',
        icon: 'table-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-10-06 22:26:55',
        updateTime: '2022-03-11 21:32:10',
        children: null,
        checked: null
      },
      {
        menuId: 141,
        parentId: 140,
        title: '基础表单',
        path: '/form/basic',
        component: 'features/form/basic',
        menuType: 0,
        sortNumber: 1,
        authority: null,
        target: '_self',
        icon: 'file-text-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:03:51',
        updateTime: '2022-03-11 15:40:07',
        children: null,
        checked: null
      },
      {
        menuId: 541,
        parentId: 140,
        title: '抽屉表单',
        path: '/form/drawer',
        component: 'features/form/drawer',
        menuType: 0,
        sortNumber: 1,
        authority: null,
        target: '_self',
        icon: 'file-text-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:03:51',
        updateTime: '2022-03-11 15:40:07',
        children: null,
        checked: null
      },
      {
        menuId: 543,
        parentId: 140,
        title: '弹窗表单',
        path: '/form/modal',
        component: 'features/form/modal',
        menuType: 0,
        sortNumber: 1,
        authority: null,
        target: '_self',
        icon: 'file-text-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:03:51',
        updateTime: '2022-03-11 15:40:07',
        children: null,
        checked: null
      },
      {
        menuId: 123,
        parentId: 122,
        title: '查询字典',
        path: null,
        component: null,
        menuType: 1,
        sortNumber: 1,
        authority: 'sys:dict:list',
        target: '_self',
        icon: null,
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2020-03-21 18:24:27',
        updateTime: '2020-03-21 18:44:42',
        children: null,
        checked: null
      },
      {
        menuId: 102,
        parentId: 101,
        title: '用户管理',
        path: '/system/user',
        component: 'system/user',
        menuType: 0,
        sortNumber: 1,
        authority: null,
        target: '_self',
        icon: 'team-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2020-02-26 12:51:55',
        updateTime: '2021-11-05 17:32:50',
        children: null,
        checked: null
      },
      {
        menuId: 130,
        parentId: 129,
        title: '查看记录',
        path: null,
        component: null,
        menuType: 1,
        sortNumber: 1,
        authority: 'sys:file:list',
        target: '_self',
        icon: null,
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2020-09-17 23:20:29',
        updateTime: '2022-03-11 15:32:53',
        children: null,
        checked: null
      },
      {
        menuId: 108,
        parentId: 107,
        title: '查询角色',
        path: null,
        component: null,
        menuType: 1,
        sortNumber: 1,
        authority: 'sys:role:list',
        target: '_self',
        icon: null,
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2020-03-13 13:30:41',
        updateTime: '2020-03-21 18:45:34',
        children: null,
        checked: null
      },
      {
        menuId: 101,
        parentId: 0,
        title: '系统管理',
        path: '/system',
        component: null,
        menuType: 0,
        sortNumber: 1,
        authority: null,
        target: '_self',
        icon: 'setting-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: '{"badge": "New", "badgeColor": "#faad14"}',
        deleted: 0,
        tenantId: 2,
        createTime: '2020-02-26 12:51:23',
        updateTime: '2022-03-11 16:47:36',
        children: null,
        checked: null
      },
      {
        menuId: 152,
        parentId: 151,
        title: '成功页',
        path: '/result/success',
        component: 'result/success',
        menuType: 0,
        sortNumber: 1,
        authority: null,
        target: '_self',
        icon: 'check-circle-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:07:40',
        updateTime: '2021-11-05 17:36:12',
        children: null,
        checked: null
      },
      {
        menuId: 148,
        parentId: 147,
        title: '项目列表',
        path: '/list/card/project',
        component: 'list/card/project',
        menuType: 0,
        sortNumber: 1,
        authority: null,
        target: '_self',
        icon: 'appstore-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:06:35',
        updateTime: '2021-11-05 17:35:53',
        children: null,
        checked: null
      },
      {
        menuId: 137,
        parentId: 136,
        title: '工作台',
        path: '/dashboard/workplace',
        component: 'features/dashboard/workbench',
        menuType: 0,
        sortNumber: 1,
        authority: null,
        target: '_self',
        icon: 'desktop-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:00:37',
        updateTime: '2021-11-05 17:34:32',
        children: null,
        checked: null
      },
      {
        menuId: 182,
        parentId: 161,
        title: '标签组件',
        path: '/extension/tag',
        component: 'extension/tag',
        menuType: 0,
        sortNumber: 1,
        authority: null,
        target: '_self',
        icon: 'tag-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-10-05 11:13:35',
        updateTime: '2022-03-11 21:32:21',
        children: null,
        checked: null
      },
      {
        menuId: 118,
        parentId: 117,
        title: '查询机构',
        path: null,
        component: null,
        menuType: 1,
        sortNumber: 1,
        authority: 'sys:org:list',
        target: '_self',
        icon: null,
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2020-03-21 18:24:21',
        updateTime: '2020-03-21 18:44:36',
        children: null,
        checked: null
      },
      {
        menuId: 149,
        parentId: 147,
        title: '应用列表',
        path: '/list/card/application',
        component: 'list/card/application',
        menuType: 0,
        sortNumber: 2,
        authority: null,
        target: '_self',
        icon: 'appstore-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:06:39',
        updateTime: '2021-11-05 17:35:54',
        children: null,
        checked: null
      },
      {
        menuId: 153,
        parentId: 151,
        title: '失败页',
        path: '/result/fail',
        component: 'result/fail',
        menuType: 0,
        sortNumber: 2,
        authority: null,
        target: '_self',
        icon: 'close-circle-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:07:41',
        updateTime: '2021-11-05 17:36:20',
        children: null,
        checked: null
      },
      {
        menuId: 138,
        parentId: 136,
        title: '分析页',
        path: '/dashboard/analysis',
        component: 'dashboard/analysis',
        menuType: 0,
        sortNumber: 2,
        authority: null,
        target: '_self',
        icon: 'bar-chart-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: '{"badge": 1}',
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:00:40',
        updateTime: '2022-03-11 16:47:54',
        children: null,
        checked: null
      },
      {
        menuId: 188,
        parentId: 172,
        title: '菜单徽章',
        path: '/example/menu-badge',
        component: 'example/menu-badge',
        menuType: 0,
        sortNumber: 2,
        authority: null,
        target: '_self',
        icon: 'appstore-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:05:26',
        updateTime: '2022-03-11 17:00:01',
        children: null,
        checked: null
      },
      {
        menuId: 171,
        parentId: 161,
        title: '弹窗扩展',
        path: '/extension/dialog',
        component: 'extension/dialog',
        menuType: 0,
        sortNumber: 2,
        authority: null,
        target: '_self',
        icon: 'block-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-10-05 11:32:12',
        updateTime: '2022-03-11 21:32:08',
        children: null,
        checked: null
      },
      {
        menuId: 156,
        parentId: 154,
        title: '404',
        path: '/exception/404',
        component: 'exception/404',
        menuType: 0,
        sortNumber: 2,
        authority: null,
        target: '_self',
        icon: 'question-circle-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:08:49',
        updateTime: '2021-11-05 17:36:52',
        children: null,
        checked: null
      },
      {
        menuId: 107,
        parentId: 101,
        title: '角色管理',
        path: '/system/role',
        component: 'features/system/pages/role',
        menuType: 0,
        sortNumber: 2,
        authority: null,
        target: '_self',
        icon: 'idcard-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2020-03-13 13:29:08',
        updateTime: '2021-11-05 17:32:55',
        children: null,
        checked: null
      },
      {
        menuId: 160,
        parentId: 158,
        title: '我的消息',
        path: '/user/message',
        component: 'user/message',
        menuType: 0,
        sortNumber: 2,
        authority: null,
        target: '_self',
        icon: 'sound-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:10:03',
        updateTime: '2021-11-05 17:37:19',
        children: null,
        checked: null
      },
      {
        menuId: 146,
        parentId: 144,
        title: '复杂列表',
        path: '/list/advanced',
        component: 'list/advanced',
        menuType: 0,
        sortNumber: 2,
        authority: null,
        target: '_self',
        icon: 'profile-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:05:26',
        updateTime: '2022-03-11 21:21:33',
        children: null,
        checked: null
      },
      {
        menuId: 140,
        parentId: 0,
        title: '表单页面',
        path: '/form',
        component: null,
        menuType: 0,
        sortNumber: 2,
        authority: null,
        target: '_self',
        icon: 'file-text-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:03:28',
        updateTime: '2021-11-05 17:34:53',
        children: null,
        checked: null
      },
      {
        menuId: 142,
        parentId: 140,
        title: '复杂表单',
        path: '/form/advanced',
        component: 'features/form/advanced',
        menuType: 0,
        sortNumber: 2,
        authority: null,
        target: '_self',
        icon: 'audit-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:03:56',
        updateTime: '2021-11-05 17:35:07',
        children: null,
        checked: null
      },
      {
        menuId: 150,
        parentId: 147,
        title: '文章列表',
        path: '/list/card/article',
        component: 'list/card/article',
        menuType: 0,
        sortNumber: 3,
        authority: null,
        target: '_self',
        icon: 'appstore-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:06:48',
        updateTime: '2021-11-05 17:35:56',
        children: null,
        checked: null
      },
      {
        menuId: 139,
        parentId: 136,
        title: '监控页',
        path: '/dashboard/monitor',
        component: 'dashboard/monitor',
        menuType: 0,
        sortNumber: 3,
        authority: null,
        target: '_self',
        icon: 'dashboard-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:03:12',
        updateTime: '2021-11-05 17:34:41',
        children: null,
        checked: null
      },
      {
        menuId: 112,
        parentId: 101,
        title: '菜单管理',
        path: '/system/menu',
        component: 'features/system/pages/menu',
        menuType: 0,
        sortNumber: 3,
        authority: null,
        target: '_self',
        icon: 'appstore-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2020-03-21 01:07:13',
        updateTime: '2022-03-11 15:37:11',
        children: null,
        checked: null
      },
      {
        menuId: 144,
        parentId: 0,
        title: '列表页面',
        path: '/list',
        component: null,
        menuType: 0,
        sortNumber: 3,
        authority: null,
        target: '_self',
        icon: 'table-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:05:07',
        updateTime: '2021-11-05 17:35:24',
        children: null,
        checked: null
      },
      {
        menuId: 163,
        parentId: 161,
        title: '文件列表',
        path: '/extension/file',
        component: 'extension/file',
        menuType: 0,
        sortNumber: 3,
        authority: null,
        target: '_self',
        icon: 'folder-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:10:58',
        updateTime: '2022-03-11 20:57:04',
        children: null,
        checked: null
      },
      {
        menuId: 157,
        parentId: 154,
        title: '500',
        path: '/exception/500',
        component: 'exception/500',
        menuType: 0,
        sortNumber: 3,
        authority: null,
        target: '_self',
        icon: 'close-circle-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:08:52',
        updateTime: '2021-11-05 17:36:59',
        children: null,
        checked: null
      },
      {
        menuId: 143,
        parentId: 140,
        title: '分步表单',
        path: '/form/step',
        component: 'features/form/step',
        menuType: 0,
        sortNumber: 3,
        authority: null,
        target: '_self',
        icon: 'one-to-one-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:04:06',
        updateTime: '2021-11-05 17:35:17',
        children: null,
        checked: null
      },
      {
        menuId: 147,
        parentId: 144,
        title: '卡片列表',
        path: '/list/card',
        component: null,
        menuType: 0,
        sortNumber: 3,
        authority: null,
        target: '_self',
        icon: 'appstore-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:05:30',
        updateTime: '2022-04-14 21:59:08',
        children: null,
        checked: null
      },
      {
        menuId: 174,
        parentId: 172,
        title: '案卷调整',
        path: '/example/document',
        component: 'example/document',
        menuType: 0,
        sortNumber: 3,
        authority: null,
        target: '_self',
        icon: 'interaction-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:15:03',
        updateTime: '2022-03-11 17:11:49',
        children: null,
        checked: null
      },
      {
        menuId: 122,
        parentId: 101,
        title: '字典管理',
        path: '/system/dictionary',
        component: 'features/system/pages/dictionary',
        menuType: 0,
        sortNumber: 4,
        authority: null,
        target: '_self',
        icon: 'profile-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2020-03-21 18:24:26',
        updateTime: '2022-03-11 17:08:47',
        children: null,
        checked: null
      },
      // {
      //   menuId: 187,
      //   parentId: 144,
      //   title: '用户信息',
      //   path: '/list/user-info/:id',
      //   component: 'list/user-info',
      //   menuType: 0,
      //   sortNumber: 4,
      //   authority: null,
      //   target: '_self',
      //   icon: 'team-outlined',
      //   color: null,
      //   hide: 1,
      //   active: null,
      //   meta: '{"active": "/list/basic"}',
      //   deleted: 0,
      //   tenantId: 2,
      //   createTime: '2021-02-02 20:05:26',
      //   updateTime: '2022-03-11 17:03:11',
      //   children: null,
      //   checked: null
      // },
      {
        menuId: 175,
        parentId: 172,
        title: '批量选择',
        path: '/example/choose',
        component: 'example/choose',
        menuType: 0,
        sortNumber: 4,
        authority: null,
        target: '_self',
        icon: 'check-square-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:15:11',
        updateTime: '2022-03-11 17:11:50',
        children: null,
        checked: null
      },
      {
        menuId: 166,
        parentId: 161,
        title: '拖拽排序',
        path: '/extension/dragsort',
        component: 'extension/dragsort',
        menuType: 0,
        sortNumber: 4,
        authority: null,
        target: '_self',
        icon: 'drag-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:11:48',
        updateTime: '2022-03-11 20:58:26',
        children: null,
        checked: null
      },
      {
        menuId: 151,
        parentId: 0,
        title: '结果页面',
        path: '/result',
        component: null,
        menuType: 0,
        sortNumber: 4,
        authority: null,
        target: '_self',
        icon: 'check-circle-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:07:33',
        updateTime: '2021-11-05 17:36:06',
        children: null,
        checked: null
      },
      {
        menuId: 117,
        parentId: 101,
        title: '班组管理',
        path: '/system/team',
        component: 'features/system/pages/team',
        menuType: 0,
        sortNumber: 5,
        authority: null,
        target: '_self',
        icon: 'bank-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2020-03-21 18:24:20',
        updateTime: '2022-03-11 17:08:48',
        children: null,
        checked: null
      },
      {
        menuId: 154,
        parentId: 0,
        title: '异常页面',
        path: '/exception',
        component: null,
        menuType: 0,
        sortNumber: 5,
        authority: null,
        target: '_self',
        icon: 'warning-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:08:44',
        updateTime: '2021-11-05 17:36:36',
        children: null,
        checked: null
      },
      {
        menuId: 176,
        parentId: 172,
        title: '内嵌页面',
        path: '/example/eleadmin',
        component: 'https://www.eleadmin.com',
        menuType: 0,
        sortNumber: 5,
        authority: null,
        target: '_self',
        icon: 'link-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-05-21 11:11:57',
        updateTime: '2022-06-18 08:34:25',
        children: null,
        checked: null
      },
      {
        menuId: 193,
        parentId: 161,
        title: '图片上传',
        path: '/extension/upload',
        component: 'extension/upload',
        menuType: 0,
        sortNumber: 5,
        authority: null,
        target: '_self',
        icon: 'picture-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2022-06-18 01:31:19',
        updateTime: '2022-06-18 01:35:50',
        children: null,
        checked: null
      },
      {
        menuId: 179,
        parentId: 161,
        title: '颜色选择',
        path: '/extension/color-picker',
        component: 'extension/color-picker',
        menuType: 0,
        sortNumber: 5,
        authority: null,
        target: '_self',
        icon: 'bg-colors-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-10-05 11:16:39',
        updateTime: '2022-03-11 21:32:15',
        children: null,
        checked: null
      },
      {
        menuId: 192,
        parentId: 172,
        title: '内嵌文档',
        path: '/example/eleadmin-doc',
        component: 'https://eleadmin.com/doc/eleadminpro/',
        menuType: 0,
        sortNumber: 6,
        authority: null,
        target: '_self',
        icon: 'link-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2022-06-18 01:30:46',
        updateTime: '2022-06-18 08:34:08',
        children: null,
        checked: null
      },
      {
        menuId: 158,
        parentId: 0,
        title: '个人中心',
        path: '/user',
        component: null,
        menuType: 0,
        sortNumber: 6,
        authority: null,
        target: '_self',
        icon: 'control-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:09:54',
        updateTime: '2021-11-05 17:37:11',
        children: null,
        checked: null
      },
      // {
      //   menuId: 129,
      //   parentId: 101,
      //   title: '文件管理',
      //   path: '/system/file',
      //   component: 'system/file',
      //   menuType: 0,
      //   sortNumber: 6,
      //   authority: null,
      //   target: '_self',
      //   icon: 'folder-outlined',
      //   color: null,
      //   hide: 1,
      //   active: null,
      //   meta: null,
      //   deleted: 0,
      //   tenantId: 2,
      //   createTime: '2020-09-17 23:19:43',
      //   updateTime: '2022-03-11 21:31:57',
      //   children: null,
      //   checked: null
      // },
      {
        menuId: 170,
        parentId: 161,
        title: '城市选择',
        path: '/extension/regions',
        component: 'extension/regions',
        menuType: 0,
        sortNumber: 6,
        authority: null,
        target: '_self',
        icon: 'apartment-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-10-05 11:25:15',
        updateTime: '2022-03-11 21:32:07',
        children: null,
        checked: null
      },
      {
        menuId: 127,
        parentId: 101,
        title: '登录日志',
        path: '/system/login-record',
        component: 'features/system/pages/loginRecord',
        menuType: 0,
        sortNumber: 7,
        authority: 'sys:login-record:view',
        target: '_self',
        icon: 'calendar-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2020-03-21 18:24:33',
        updateTime: '2021-11-05 17:33:28',
        children: null,
        checked: null
      },
      {
        menuId: 164,
        parentId: 161,
        title: '打印插件',
        path: '/extension/printer',
        component: 'extension/printer',
        menuType: 0,
        sortNumber: 7,
        authority: null,
        target: '_self',
        icon: 'printer-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:11:00',
        updateTime: '2022-03-11 20:58:48',
        children: null,
        checked: null
      },
      {
        menuId: 161,
        parentId: 0,
        title: '扩展组件',
        path: '/extension',
        component: null,
        menuType: 0,
        sortNumber: 7,
        authority: null,
        target: '_self',
        icon: 'appstore-add-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:10:52',
        updateTime: '2021-11-05 17:37:51',
        children: null,
        checked: null
      },
      {
        menuId: 165,
        parentId: 161,
        title: 'excel插件',
        path: '/extension/excel',
        component: 'extension/excel',
        menuType: 0,
        sortNumber: 8,
        authority: null,
        target: '_self',
        icon: 'table-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:11:03',
        updateTime: '2022-03-11 20:58:50',
        children: null,
        checked: null
      },
      {
        menuId: 172,
        parentId: 0,
        title: '常用实例',
        path: '/example',
        component: null,
        menuType: 0,
        sortNumber: 8,
        authority: null,
        target: '_self',
        icon: 'compass-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:15:01',
        updateTime: '2021-11-05 17:39:02',
        children: null,
        checked: null
      },
      {
        menuId: 128,
        parentId: 101,
        title: '操作日志',
        path: '/system/operation-record',
        component: 'features/system/pages/operationRecord',
        menuType: 0,
        sortNumber: 8,
        authority: 'sys:operation-record:view',
        target: '_self',
        icon: 'file-search-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2020-03-21 18:24:34',
        updateTime: '2021-11-05 17:33:40',
        children: null,
        checked: null
      },
      // {
      //   menuId: 135,
      //   parentId: 101,
      //   title: '用户信息',
      //   path: '/system/user-info',
      //   component: 'system/user-info',
      //   menuType: 0,
      //   sortNumber: 9,
      //   authority: null,
      //   target: '_self',
      //   icon: 'team-outlined',
      //   color: null,
      //   hide: 1,
      //   active: null,
      //   meta: '{"active": "/system/user"}',
      //   deleted: 0,
      //   tenantId: 2,
      //   createTime: '2021-05-21 11:07:54',
      //   updateTime: '2022-06-19 17:45:09',
      //   children: null,
      //   checked: null
      // },
      {
        menuId: 177,
        parentId: 0,
        title: '获取授权',
        path: 'https://eleadmin.com/goods/9',
        component: null,
        menuType: 0,
        sortNumber: 9,
        authority: null,
        target: '_self',
        icon: 'sketch-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:15:18',
        updateTime: '2022-03-11 21:31:43',
        children: null,
        checked: null
      },
      {
        menuId: 183,
        parentId: 161,
        title: '滚动数字',
        path: '/extension/count-up',
        component: 'extension/count-up',
        menuType: 0,
        sortNumber: 9,
        authority: null,
        target: '_self',
        icon: 'ellipsis-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-10-05 11:29:03',
        updateTime: '2022-03-11 21:32:24',
        children: null,
        checked: null
      },
      {
        menuId: 168,
        parentId: 161,
        title: '视频播放',
        path: '/extension/player',
        component: 'extension/player',
        menuType: 0,
        sortNumber: 10,
        authority: null,
        target: '_self',
        icon: 'youtube-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:13:49',
        updateTime: '2022-03-11 21:01:36',
        children: null,
        checked: null
      },
      {
        menuId: 167,
        parentId: 161,
        title: '地图组件',
        path: '/extension/map',
        component: 'extension/map',
        menuType: 0,
        sortNumber: 11,
        authority: null,
        target: '_self',
        icon: 'environment-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:11:49',
        updateTime: '2022-03-11 21:01:29',
        children: null,
        checked: null
      },
      {
        menuId: 169,
        parentId: 161,
        title: '富文本框',
        path: '/extension/editor',
        component: 'extension/editor',
        menuType: 0,
        sortNumber: 12,
        authority: null,
        target: '_self',
        icon: 'font-size-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-02-02 20:13:57',
        updateTime: '2022-03-11 17:28:11',
        children: null,
        checked: null
      },
      {
        menuId: 189,
        parentId: 161,
        title: 'markdown',
        path: '/extension/markdown',
        component: 'extension/markdown',
        menuType: 0,
        sortNumber: 13,
        authority: null,
        target: '_self',
        icon: 'picLeft-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-10-10 16:21:45',
        updateTime: '2022-03-11 21:32:45',
        children: null,
        checked: null
      },
      {
        menuId: 186,
        parentId: 161,
        title: '表格下拉',
        path: '/extension/table-select',
        component: 'extension/table-select',
        menuType: 0,
        sortNumber: 14,
        authority: null,
        target: '_self',
        icon: 'table-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-10-10 16:21:45',
        updateTime: '2022-04-01 10:21:49',
        children: null,
        checked: null
      },
      {
        menuId: 184,
        parentId: 161,
        title: '二维码',
        path: '/extension/qr-code',
        component: 'extension/qr-code',
        menuType: 0,
        sortNumber: 15,
        authority: null,
        target: '_self',
        icon: 'qrcode-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2021-10-05 11:29:56',
        updateTime: '2022-04-04 17:47:29',
        children: null,
        checked: null
      },
      {
        menuId: 194,
        parentId: 161,
        title: '条形码',
        path: '/extension/bar-code',
        component: 'extension/bar-code',
        menuType: 0,
        sortNumber: 20,
        authority: null,
        target: '_self',
        icon: 'barcode-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2022-06-18 01:31:27',
        updateTime: '2022-06-18 01:35:50',
        children: null,
        checked: null
      },
      {
        menuId: 600,
        parentId: 0,
        title: '表格页面',
        path: '/extension/bar-code',
        component: null,
        menuType: 0,
        sortNumber: 20,
        authority: null,
        target: '_self',
        icon: 'barcode-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2022-06-18 01:31:27',
        updateTime: '2022-06-18 01:35:50',
        children: null,
        checked: null
      },
      {
        menuId: 601,
        parentId: 600,
        title: '轻量筛选表格',
        path: '/table/lightWeight',
        component: 'features/table/lightWeight',
        menuType: 0,
        sortNumber: 20,
        authority: null,
        target: '_self',
        icon: 'barcode-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2022-06-18 01:31:27',
        updateTime: '2022-06-18 01:35:50',
        children: null,
        checked: null
      },
      {
        menuId: 602,
        parentId: 600,
        title: '多选表格',
        path: '/table/choice',
        component: 'features/table/choice',
        menuType: 0,
        sortNumber: 20,
        authority: null,
        target: '_self',
        icon: 'barcode-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2022-06-18 01:31:27',
        updateTime: '2022-06-18 01:35:50',
        children: null,
        checked: null
      },
      {
        menuId: 603,
        parentId: 600,
        title: '编辑表格',
        path: '/table/edit',
        component: 'features/table/edit',
        menuType: 0,
        sortNumber: 20,
        authority: null,
        target: '_self',
        icon: 'barcode-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2022-06-18 01:31:27',
        updateTime: '2022-06-18 01:35:50',
        children: null,
        checked: null
      },
      {
        menuId: 604,
        parentId: 600,
        title: '嵌套表格',
        path: '/table/nested',
        component: 'features/table/nested',
        menuType: 0,
        sortNumber: 20,
        authority: null,
        target: '_self',
        icon: 'barcode-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2022-06-18 01:31:27',
        updateTime: '2022-06-18 01:35:50',
        children: null,
        checked: null
      },
      {
        menuId: 605,
        parentId: 600,
        title: '基础表格',
        path: '/table/normal',
        component: 'features/table/normal',
        menuType: 0,
        sortNumber: 20,
        authority: null,
        target: '_self',
        icon: 'barcode-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2022-06-18 01:31:27',
        updateTime: '2022-06-18 01:35:50',
        children: null,
        checked: null
      },
      {
        menuId: 606,
        parentId: 600,
        title: '左右布局表格',
        path: '/table/retinue',
        component: 'features/table/retinue',
        menuType: 0,
        sortNumber: 20,
        authority: null,
        target: '_self',
        icon: 'barcode-outlined',
        color: null,
        hide: 0,
        active: null,
        meta: null,
        deleted: 0,
        tenantId: 2,
        createTime: '2022-06-18 01:31:27',
        updateTime: '2022-06-18 01:35:50',
        children: null,
        checked: null
      }
    ],
    pageSize: 10,
    current: 1,
    total: 831,
    totalPage: 84
  },
  // 返回值也可以是对象形式
  'GET /api/users/1': { id: 1, name: 'foo' }
};
